...
public class RequestHelper {
	 ...
	 * returns home data /api/profile-info -> returns profile data
	 */
	...
	public void processUpdate(HttpServletRequest request, HttpServletResponse response)
		...
		if (path.startsWith("/api/")) {
			...
			case "reimbursement/bulk": {
				...
				String strApproverId = jObj.getString("approved_by");
				...
				if (!reimbList.isEmpty()) {
					...
					reimbursementDelegate.updateReimbursementStatus(request, response, reimbList, 
							status, Integer.parseInt(strApproverId));
